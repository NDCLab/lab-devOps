        ...
        if sub_folder.startswith("sub-"):
            ...
            if isdir(join(out_location,sub_folder,session,"eeg")):
                for f in listdir(join(out_location,sub_folder,session,"eeg")):
                    file_re = re.match('^MADE_preprocessing_report_(.+?)(_ERROR_incomplete)?\.csv$', f)
                    if file_re:
                        task = file_re.group(1)
                        error_seen = file_re.group(2)
...
#########################
    ...
    #for task, subs in eeg_tasks_incomplete_subjects.items():
    #    #colname = task + "_preprocessing_" + session + "_e1_complete"
    #    colname = task + "_preprocessing_finished_" + session + "_e1"
    #    tracker_df.loc[subs, colname] = 0
