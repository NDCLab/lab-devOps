...
def analyze_recall_periods(input_dir: str, output_dir: str):
    recall_subs = os.listdir(input_dir)
    ...
    for sub in recall_subs:
        ...
        all_sub_passages = [
            ...
            if "all-cols" in f and not f.startswith(".")
        ]
        ...
        for phrase in recalled_phrases:
            for filename in all_sub_passages:
                path = os.path.join(sub_data_dir, filename)
                passage_df = pd.read_csv(path).sort_values(by=["SyllableID"])
                ...
                cleaned_words = (
                    # Drop duplicate words, keeping the first entry
                    passage_df.drop_duplicates(subset=["WordID"], keep="first")[
                        "CleanedWord"
                    ]
                    .dropna()
                    .str.lower()
                ...
                for start_idx in start_indices:
                    ...
                    occurrence_info["actualPhrase"] = " ".join(
                        phrase_entries.drop_duplicates(subset=["WordID"], keep="first")[
                            "CleanedWord"
                        ]
                    )
                    ...
                    occurrence_info["actualExtendedPhrase"] = " ".join(
                        five_syll_entries.drop_duplicates(
    ...
    recall_dir = os.path.join(output_dir, "recall_analysis")
    os.makedirs(recall_dir, exist_ok=True)

    dt_now = datetime.datetime.now().strftime("%Y_%m_%d-%H%M")

    all_rp1_df = pd.DataFrame(rp1_data)
    all_rp1_df.to_csv(os.path.join(recall_dir, f"{dt_now}-rp1_data.csv"), index=False)

    all_rp2_df = pd.DataFrame(rp2_data)
    all_rp2_df.to_csv(os.path.join(recall_dir, f"{dt_now}-rp2_data.csv"), index=False)
