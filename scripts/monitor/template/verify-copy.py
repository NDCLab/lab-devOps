            ...
            if isdir(join(raw, ses, datatype)):
                for subject in listdir(join(raw, ses, datatype)):
                    ...
                    if len(obs_files) > numfiles:
                        print("Error: number of data files in subject folder", subject, obs_files, "greater than the expected number", numfiles)
                    elif len(obs_files) < numfiles:
                        print("Error: number of data files in subject folder", subject, obs_files, "less than the expected number", numfiles)
                    for raw_file in listdir(join(raw, ses, datatype, subject)):
                        ...
                        if file_re:
                            ...
                            #possible_exts = sum([ext.split('|') for ext in fileexts], [])
                            if file_re.group(9) not in possible_exts:
                            ...
                            if file_re.group(3) not in dd_dict.keys():
                                print("Error: taskname", file_re.group(3), "does not match any datadict tasknames, in file:", join(raw, ses, datatype, subject, raw_file))
                            ...
                            if file_re.group(3) == "":
                                print("Error: taskname missing from file:", join(raw, ses, datatype, subject, raw_file))
                            if file_re.group(6) == "":
