...
import pandas as pd
from hallmonitor.hmutils import Identifier, get_variable_datatype
...
def get_redcap_columns(datadict_df, session):
    ...
    for _, row in df.iterrows():
        ...
        if "file:" in prov and "variable:" in prov:
            ...
            rc_variable = str(prov[idx + 1]).strip('";,')
            if rc_variable == "":
        ...
        for ses_tag in allowed_suffixes:
            ...
            if surv_match and "redcap_data" in row["dataType"]:
                ...
                multiple_report_tag = (
                    "" if not surv_match.group(4) else surv_match.group(4)
                )
                surv_esp = (
                    surv_match.group(1)
                    + "es"
...
def get_tasks(datadict_df: pd.DataFrame):
    ...
    variables_without_ext = datadict_df[datadict_df["expectedFileExt"].isna()][
        "variable"
    ].to_list()
    task_vars += variables_without_ext

    var_subdf = datadict_df[datadict_df["variable"].isin(task_vars)]
    for _, row in var_subdf.iterrows():
        if row["variable"] in task_vars:
            if isinstance(row["dataType"], str) and isinstance(
                row["expectedFileExt"], str
            ):
                tasks_dict[row["variable"]] = [
                    row["dataType"],
                    row["expectedFileExt"],
                    row["allowedSuffix"],
                ]
                ...
                print(
                    c.RED
                    + "Error: Must have dataType, expectedFileExt, and allowedSuffix fields in datadict for ",
                    row["variable"],
                    ", skipping." + c.ENDC,
                )
...
def get_IDs(datadict_df: pd.DataFrame, checked_path: str):
    """Returns list of subject IDs from the Redcap and column specified in the "id" row in the datadict"""
    id_desc = datadict_df.set_index("variable").loc["id", "provenance"].split(" ")
    ...
    for i in id_desc:
        if "file:" in i:
            ...
            idx = id_desc.index(i) + 1
            found_var = id_desc[idx].strip("\"';,()")
    if id_rc is None or found_var is None:
        sys.exit("Can't find redcap column to read IDs from in datadict")

    rc_dir = os.path.join(checked_path, "redcap")
    redcap_files = [
        os.path.join(checked_path, "redcap", f)
        for f in os.listdir(rc_dir)
        if os.path.isfile(os.path.join(rc_dir, f))
    ]
    ...
    for redcap in redcap_files:
        rc_re = re.fullmatch(
            id_rc + r".*_data_(\d{4}-\d{2}-\d{2}_\d{4}).*",
            os.path.basename(redcap).lower(),
        )
        if rc_re:
    ...
    if consent_redcap is None:
        sys.exit("Can't find" + id_rc + "redcap to read IDs from")
    consent_redcap = pd.read_csv(consent_redcap, index_col=found_var)
    ids = consent_redcap.index.tolist()
    return ids


def get_study_no(datadict_df: pd.DataFrame):
    ...
    return intervals[0][:2]  # first two digits should be study no.


def fill_combination_columns(tracker_df: pd.DataFrame, dd_df: pd.DataFrame):
    """Fills in combination columns, i.e. checks for the presence of one or more combination variables"""
    combos_dict = dict()
    combination_df = dd_df[dd_df["dataType"] == "combination"]
    ...
    for key, cols in combos_dict.items():
        if len(cols) == 0:
            print(
                c.RED
                + "Error: columns to combine not found for combination variable: "
                + key
                + ", can't update column."
                + c.ENDC
            )
                    ...
                    for _, rc_row in rc_df.iterrows():
                        ...
                        if child_id_match is not None:
                            ...
                            if rc_row[col] in {1, 2}:
                                ...
                                print(
                                    "Error: unknown value seen for parent language, should be 1 for English and 2 for Spanish."
                                )
    return parent_info
...
def main(
    ...
    passed_ids = [(Identifier.from_str(s), 1) for s in passed_id_list.split(",")]
    failed_ids = [(Identifier.from_str(s), 0) for s in failed_id_list.split(",")]
    ...
    redcheck_columns, allowed_duplicate_columns = get_redcap_columns(df_dd, session)
    ids = get_IDs(df_dd, checked_path)
    ...
    new_subjects = set(ids).difference(tracker_ids)
    ...
    if redcaps[0] != "none":
        ...
        for expected_rc in redcheck_columns.keys():
            ...
            vals = (
                pd.read_csv(redcap_path, header=None, nrows=1).iloc[0, :].value_counts()
            )
            ...
            if any(vals.values != 1):
                ...
                sys.exit(
                    c.RED
                    + "Error: Duplicate columns found in redcap "
                    + redcap_path
                    + ": "
                    + ", ".join(dupes)
                    + ". Exiting"
        ...
        for expected_rc in redcheck_columns.keys():
            ...
            for key, value in redcheck_columns[expected_rc].items():
                ...
                if (
                    ...
                    or key.startswith("assent")
                    or key.startswith("id_column")
                ):
                    continue
                if (
            ...
            for index, row in rc_df.iterrows():
                if (
                    isinstance(index, float) or isinstance(index, int)
                ) and not math.isnan(index):
                    id = int(row.name)
                else:
                    print(
                ...
                if child == "true":
                    if re.search(study_no + "[089](\d{4})", str(id)):
                        child_id = (
                            study_no
                            + "0"
                            + re.search(study_no + "[089](\d{4})", str(id)).group(1)
                        )
                        child_id = int(child_id)
                    else:
                        print(
            ...
            for col in rc_df.columns:
                if col.endswith(completed):
                    all_redcap_columns.setdefault(col, []).append(
                        all_redcap_paths[expected_rc]
                    )

        parent_info = parent_columns(
            df_dd, tracker_df, study_no, session, all_redcap_paths
        )
        ...
        for col, rcs in all_redcap_columns.items():
            if (
                len(all_redcap_columns[col]) > 1
                and col not in allowed_duplicate_columns
            ):
        ...
        sys.exit(
            "Can't find redcaps in " + dataset + "/sourcedata/raw/redcap, skipping "
        )
